...
export function ProductsToolbar({
    ...
    const hasFilters = search !== '' || category !== 'all' || status !== 'all' || sort !== 'name-asc';
    ...
    return (
        <div className="flex flex-col gap-3 border-b border-slate-200 bg-slate-50/70 p-4 lg:flex-row lg:items-center">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3 lg:flex lg:shrink-0">
                ...
                <Select id="product-category-filter" value={category} onChange={(event) => onCategoryChange(event.target.value)} className="h-9 bg-white">
                    <option value="all">All categories</option>
                    {categories.map((item) => <option key={item} value={item}>{item}</option>)}
                </Select>

                <label className="sr-only" htmlFor="product-status-filter">Status</label>
                <Select id="product-status-filter" value={status} onChange={(event) => onStatusChange(event.target.value)} className="h-9 bg-white">
                ...
                <Select id="product-sort" value={sort} onChange={(event) => onSortChange(event.target.value)} className="h-9 bg-white">
